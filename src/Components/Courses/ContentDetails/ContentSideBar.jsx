...
import { TOPICS, CHAPTERS, MODULES, courseItems, COURSES } from "../../../Utils/constant";
...
const ContentSideBar = () => {
  ...
  useEffect(() => {
    ...
    if (topic) {
      const chapterData = CHAPTERS.find((chapter) => chapter.id === topic.chapterId);
      const moduleData = chapterData ? MODULES.find((module) => module.id === chapterData.moduleId) : null;

      if (moduleData) {
        const relatedChapters = CHAPTERS.filter((ch) => ch.moduleId === moduleData.id);
  ...
  const topic = TOPICS.find((topic) => topic.id === parseInt(topicId));
  const chapterData = topic ? CHAPTERS.find((ch) => ch.id === topic.chapterId) : null;
  const moduleData = chapterData ? MODULES.find((mod) => mod.id === chapterData.moduleId) : null;
  const course = moduleData ? COURSES.find((c) => c.id === moduleData.courseId) : null;
  const category = course ? courseItems.find((category) => category.id === course.categoryId).name.toLowerCase().replace(/\s+/g, "-") : "";
  const courseTitle = course ? course.title.toLowerCase().replace(/\s+/g, "-") : "";
  ...
  return (
    <div className="w-80 h-full bg-gray-900 text-white p-4 shadow-xl overflow-y-auto transition-all duration-300">
      ...
      {/* Header */}
      <h2 className="text-lg font-semibold mb-4 text-gray-300">List of Modules</h2>
        ...
        <div key={chapter.id} className="mb-3">
          ...
          {expandedChapterId === chapter.id && (
            <div className="bg-gray-900 text-gray-300 p-4 mt-4 rounded-md border-t border-gray-700 mb-4">
              {TOPICS.filter((topic) => topic.chapterId === chapter.id).map((topic) => (
                <Link
                  key={topic.id}
